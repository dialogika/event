...
const handleIndexFormSubmission = async (event) => {
  ...
  const whatsapp = iti_webinar.getNumber();
  ...
  const whatsappModalFallback = new bootstrap.Modal(document.getElementById("whatsappFallbackModalWebinar"));
  ...
  try {
    ...
    openWhatsAppInvite("https://chat.whatsapp.com/H4jz5SoO4lX5WTD2VvBaCC?mode=ems_copy_t");
...
const handlePresensiBtn = async (event) => {
  ...
  const loadingSpinner = document.getElementById("loadingSpinner");
  const whatsAppInput = iti_presensi.getNumber();
...
const handleEventSenam = async (event) => {
  ...
  const loadingSpinner = document.getElementById("loadingSpinner");
  const whatsAppInput = iti_senam.getNumber();
  const namaInput = document.getElementById("input-nama-senam").value.trim();
  const emailInput = document.querySelector('input[name="Email"]');
  const universityInput = document.getElementById("university");
  const infoInput = document.getElementById("formInfo");
  // const eventTitle = document.getElementById("eventTitle").innerText.trim();
  const description = "Mengirim data presensi peserta event webinar !";
  const whatsappModalFallback = new bootstrap.Modal(document.getElementById("whatsappFallbackModalSenam"));
